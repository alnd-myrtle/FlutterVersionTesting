...
//
const CustomAddVillas = ({ show, setShow, villasData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://api.waren-d.com/api/villas",
        formData,
        {
          headers: {
  ...
  useEffect(() => {
    fetchData();
  }, [houseOldNumberfilter]);

  const fetchData = async () => {
    console.log(houseOldNumberfilter);
    const res = await axios.get(
      `https://api.waren-d.com/api/SelfHouse1`,
      {
